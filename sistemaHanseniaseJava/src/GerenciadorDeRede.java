...
public class GerenciadorDeRede {
		...
		try {
			...
			for (int i = 0; i < parametros.length - 1; ++i) {				
				...
				if (parametros[i] == 0) {
					...
					throw new Exception("Parametro indisponivel. Esperado: <" + dicionario[i].length + "  Recebido"
							+ parametros[i]);
	...
	public String avaliar() {
		...
		return "{\"sr\":" + (rede.finding_controle() * 100) + ", \"r1\":" + (rede.finding_rr() * 100) + ", \"r2\":"
				+ (rede.finding_enh() * 100) + "}";
	}
}
