...
const issuesAvailableByDatabase: number[][] = [
...
export class DatabaseFilter{
    ...
    static convertDatabaseIdForNeticaCode(){
        ...
        if(id === 0)
            return issuesAvailableByDatabase.length - 1;
        else
            return id - 1;
    }
    ...
    static questionIsAllowed(questionId: number){
        return issuesAvailableByDatabase[DatabaseFilter.databaseId].includes(questionId)
    }
    static refreshDatabase(databaseId: number){
        DatabaseFilter.setDatabaseId(databaseId)
        if(databaseId > issuesAvailableByDatabase.length && databaseId < 0)
            throw new Error('Database not allowed. Id: ' + databaseId)
        const issuesAlowed = issuesAvailableByDatabase[databaseId]
        for(const [index, action] of DatabaseFilter.answerListeners.entries()){
            const questionAllowed = issuesAlowed.includes(index)
            if(action)
